...
async function build() {
  ...
  // Generate favicon binary module for the Worker to serve at /favicon.png
  const faviconBytes = readFileSync("web/assets/favicon-64.png");
  const faviconArray = `new Uint8Array([${Array.from(faviconBytes).join(",")}])`;
  writeFileSync(
    "src/favicon.ts",
    `// AUTO-GENERATED by web/build.ts — do not edit\nexport const FAVICON_PNG: Uint8Array = ${faviconArray};\n`,
  );
  console.log("Generated src/favicon.ts");
}
